...
'''
README
This file tests the message_unreact function

Assume that react_id is valid from 1 to 10
...
def test_message_unreact_userNotInChannel():
    ...
    with pytest.raises(AccessError):
        voidDict = message_unreact(nonAdminToken, message_id,1)


def test_message_unreact_invalidMessageID():
	adminToken = create_admin_token()
	channelID = create_public_channel_id()
	...
	with pytest.raises(ValueError):
		voidDict = message_unreact(adminToken, message_id,react_id)
...
def test_message_unreact_invalidReactID():
	...
	invalid_react_id = -1
	message_react(adminToken, message_id,1)

	with pytest.raises(ValueError):
		voidDict = message_unreact(adminToken, message_id,invalid_react_id)
	


def test_message_unreact_alreadyUnreact():
...
def test_message_unreact_success():
	...
	react_id = 1
	message_react(adminToken, message_id,react_id)

	voidDict = message_unreact(adminToken, message_id,react_id)

	assert(voidDict==None)
